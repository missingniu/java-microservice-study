    ...
    <el-table :data="pageInfo.rows" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
      ...
      <el-table-column label="操作" >
        <template slot-scope="scope">
          ...
          <el-button type="danger" size="small" @click="handleBusiness(scope.$index, scope.row)">转化商机</el-button>
          <el-button type="danger" size="small" v-if ="scope.row.state==1" @click="handleClueActivity(scope.$index, scope.row)" >分配</el-button>
...
<!--    批量分配-->
    <el-dialog title="批量分配" :visible.sync="batchAssignVisible" :close-on-click-modal="false">
      <el-form :model="batchAssign" label-width="80px" ref="batchAssginForm">
