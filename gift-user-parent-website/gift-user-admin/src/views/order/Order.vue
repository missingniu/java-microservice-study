<template>
  <section>
    ...
    <el-table :data="pageInfo.rows" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
      ...
      <el-table-column prop="state" label="状态"  width="210" sortable>
        <template slot-scope="scope">
          <span v-if="scope.row.state==0" style="color:grey">代付款</span>
          <span v-if="scope.row.state==1" style="color:skyblue">已付定金</span>
          <span v-if="scope.row.state==3" style="color:lawngreen">支付完成</span>
          <span v-if="scope.row.state==2" style="color:lawngreen">已经完成</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" >
        <template slot-scope="scope">
          ...
          <el-button type="danger" v-if="scope.row.state == 1 || scope.row.state == 3" size="small" @click="pay(scope.$index, scope.row)">付尾款</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!--    支付尾款-->
    <el-dialog title="支付尾款" :visible.sync="payRemainVisible" :close-on-click-modal="false">
      <el-form :model="payRemain" label-width="80px" :rules="saveFormRules" ref="saveForm">
        <el-form-item label="编号">
          <el-input v-model="payRemain.sn" disabled auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="商品名称">
          <el-input v-model="payRemain.productName" disabled auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="订单金额">
          <el-input v-model="payRemain.price" disabled auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="定金金额">
          <el-input v-model="payRemain.deposit"  auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="待付尾款">
          <el-input v-model="payRemain.payMoney" disabled auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="付款方式">
          <el-select v-model="payRemain.payMethod" placeholder="请选择">
            <el-option
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click.native="saveFormVisible = false">取消</el-button>
        <el-button type="primary" @click.native="saveMoney" :loading="addLoading">提交</el-button>
      </div>
    </el-dialog>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    saveBusiness(){
      this.$http.post("/order/business",this.business)
      .then(result=>{
        result = result.data;
        if(result.success){
          this.$message({
    ...
    saveBatchAssign(){
      ...
      // 将线索id和营销人员一块传到后端
      this.$http.put("/order/batch/assign",this.batchAssign)
          .then(result => {
            result = result.data;
            if(result.success){
              this.$message({
    ...
    getAllMethods(){
      this.$http.get("/dictionaryItem/sn")
      .then(result=>{
        result=result.data;
        console.log(result);
        this.payMethods = result.resultObject;
      })
    },
    //保存
    saveSubmit: function () {
    ...
    pay(index,row){
      this.payRemainVisible= true;
      this.payRemain.orderId = row.id;
      this.payRemain.productName = row.productName;
      this.payRemain.sn = row.sn;
      this.payRemain.price = row.price;
      this.getDepositByOrderId(row.id);
      this.getAllMethods();
    },
    getDepositByOrderId(orderId){
      this.$http.get("/deposit/orderId/"+orderId)
          .then(result=>{
            result = result.data;
            console.log(result);
            if(result.success){
              this.payRemain.deposit = result.resultObject.deposit;
              this.payRemain.payMoney = this.payRemain.price - this.payRemain.deposit;//尾款等于总价格减去定金价格
            }
          })
    },
    saveMoney(){
        this.$http.put("/order/pay",this.payRemain)
      .then(result=>{
        result = result.data;
        console.log(result);
        if(result.success){
