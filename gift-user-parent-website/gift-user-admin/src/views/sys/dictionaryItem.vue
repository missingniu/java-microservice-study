<template>
  <section>
    ...
    <el-table :data="pageInfo.rows" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
      ...
      <el-table-column prop="title" label="标签" width="180" sortable>
      </el-table-column>
      <el-table-column prop="value" label="值" width="180" sortable>
      </el-table-column>
      <el-table-column prop="sequence" label="序列" width="180" sortable>
      </el-table-column>
      <el-table-column prop="intro" label="描述" width="230" sortable>
      </el-table-column>
      <el-table-column prop="parent.title" label="父级字典" width="180" sortable>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="saveFormVisible" :close-on-click-modal="false">
      <el-form :model="saveForm" label-width="80px" :rules="saveFormRules" ref="saveForm">
        ...
        <el-form-item label="序列" prop="sequence">
          <el-input v-model="saveForm.sequence" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="描述" prop="intro">
          <el-input v-model="saveForm.intro" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="父级字典" prop="parent">
          <el-select v-model="saveForm.parent.id" clearable value-key="id" placeholder="请选择">
            <!--下拉选项
              :key=""  //唯一标识
              :label 选择之后展示到选择框中的值
...
export default {
  data() {
    return {
      ...
      saveFormRules: {
        sn: [
          { required: true, message: '请输入唯一标识', trigger: 'blur' }//blur当鼠标失去焦点的时候
        ]
      },
      //新增界面数据
     saveForm: {
  ...
  methods: {
    ...
    getDictionary() {
      ...
      this.$http.get("/dictionary",this.query)
          .then(result => {
            result = result.data;
            console.log(result);  // 后端返回的resultObj里面,包含rows(当前页数据) total 总条数
            this.allDictionary = result.resultObject;
    ...
    handleEdit: function (index, row) {
      ...
      if(!this.saveForm.parent){//需要个初始值，防止回显得时候出错
