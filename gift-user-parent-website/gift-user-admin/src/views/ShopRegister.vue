  ...
  <div>
    ...
    <el-form :model="tenant" ref="tenantForm" :rules="formRules" label-position="left" label-width="100px" class="demo-ruleForm login-container">
      ...
      <el-form-item prop="employee.username" label="账号">
        <el-input type="text" v-model="tenant.employee.username" auto-complete="off" placeholder="请输入账号！"></el-input>
      </el-form-item>
      <el-form-item prop="employee.tel" label="手机号码">
        <el-input type="text" v-model="tenant.employee.tel" auto-complete="off" placeholder="请输入电话！"></el-input>
      </el-form-item>
      <el-form-item prop="employee.email" label="电子邮件">
        <el-input type="text" v-model="tenant.employee.email" auto-complete="off" placeholder="请输入邮件！"></el-input>
      </el-form-item>
      <el-form-item prop="employee.password" label="密码">
        <el-input type="password" v-model="tenant.employee.password" auto-complete="off" placeholder="请输入密码！"></el-input>
      </el-form-item>
      <el-form-item prop="employee.confirmPassword" label="确认密码">
        <el-input type="password" v-model="tenant.confirmPassword" auto-complete="off" placeholder="请输入确认密码！"></el-input>
      </el-form-item>
...
export default {
  data() {
    var validatePass2 = (rule, value, callback) => {
      ...
      } else if (value !== this.tenant.employee.password) {
      ...
      this.$refs.tenantForm.validate((valid) => {
        ...
        if (valid) {
          this.$confirm('确认入驻吗？', '提示', {}).then(() => {
            ...
            // 为了后台好接收，封装一个对象放到里面
            //判断是否有id有就是修改,否则就是添加
            this.$http.post("/sys/tenant/settlement",para).then((res) => {
              if(res.data.success){
                this.$message({
                  message: '操作成功!',
                  type: 'success'
